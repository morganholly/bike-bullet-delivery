...
func _ready():
	...
	MissionManager.mission_completed.connect(mission_events)
	MissionManager.mission_phase_completed.connect(mission_events_phase)
...
func _start_mission_spawning():
	print("LEVEL:_start_mission_spawning")
...
func _on_mission_spawn_timer_timeout():
	...
	if MissionManager.tutorial == false:
...
func _on_mission_completed(mission_id: String) -> void:
	print("MISSION: (level) _on_mission_completed")
...
func mission_events(mission_id):
	#a mission has just finished. Do a thing.
	print("mission count: ",MissionManager.completed_missions.size())
	if MissionManager.completed_missions.size()==2:
		UIManager.show_prompt_timer("RIDER BONUS: dispensing weaponry",3.0)
		var pistol = PistolPickup.instantiate()
		add_child(pistol)
		pistol.position = Vector3(-46,3,-123)
	elif MissionManager.completed_missions.size()==5:
		pass
	pass 
	
func mission_events_phase(mission_id, phase_index):
	#a phase has finished. Do a thing
	if MissionManager.completed_missions.size()==3:
		#move boomguy over to the ufo
		var boomguy = $Boomguys/Boomguy
		boomguy.global_position = marker1.global_position
		#create ufo
		$"Billboard Alieny/Alieny_ufo".uncloak()
		#create alien reinforcements
		var alien = GreenAlien.instantiate()
		add_child(alien)
		alien.transform=$"Billboard Alieny/spawn_alien".transform
		
		alien = GreenAlien.instantiate()
		add_child(alien)
		alien.transform=$"Billboard Alieny/spawn_alien2".transform
		
		alien = GreenAlien.instantiate()
		add_child(alien)
		alien.transform=$"Billboard Alieny/spawn_alien3".transform
		
		alien = GreenAlien.instantiate()
		add_child(alien)
		alien.transform=$"Billboard Alieny/spawn_alien4".transform
		
	elif MissionManager.completed_missions.size()==5:
		#complete the game
		get_tree().change_scene_to_file("res://cutscenes/slideshow/win_slideshow.tscn")
		pass
	pass
